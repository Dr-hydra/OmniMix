...
RetryCacheCheck:
        ...
        Catch ex As Exception
            ...
            Catch
            End Try
            MsgBox(ex.GetDisplay(True) & vbCrLf & "OmniMix Player 所在路径：" & If(String.IsNullOrEmpty(FilePath), "获取失败", FilePath), MsgBoxStyle.Critical, "OmniMix Player 初始化错误")
    ...
    Private Shared Sub WaitingMutex(e As StartupEventArgs)
        ...
        Try
            ...
            PclMutex = New Mutex(True, "OmniMixPlayer_SingletonMutex", MutexCreatedNew)
            If MutexCreatedNew Then Return
            Logger.Warn("已有一个 OmniMix Player 实例正在运行")
            '等待已有的 OmniMix Player 退出
            If e.Args.Length > 0 AndAlso e.Args(0) = "--wait" Then
                ...
                End Try
            End If
            '将已有的 OmniMix Player 窗口拖出来
            Dim WindowHwnd As IntPtr = FindWindow(Nothing, "OmniMix Player")
    ...
    '动态 DLL 加载
    Private Sub New() '这里必须尽早调用，且不能使用任何库，否则加载 OmniMixCore 就会导致崩溃
        Static Prefixes As String() = {"NAudio", "Newtonsoft.Json", "Ookii.Dialogs.Wpf", "Imazen.WebP", "CacheCow.Common", "CacheCow.Client.FileStore", "CacheCow.Client", "System.Net.Http.Formatting", "OmniMixCore.Wpf", "OmniMixCore"}
        ...
        End Function
    End Sub
    Private Shared Sub ExtractLibwebp() '这个方法会调用 ModBase，进而调用 OmniMixCore，所以不能放在 New 里
