...
#Region "声明"
    ...
    '下列版本信息由更新器自动修改
    Public Const VersionBaseName As String = "4.4.2" '显示用版本名
    Public Const CommitHash As String = "" 'Commit Hash，由 GitHub Workflow 自动替换
#If RELEASE Then
    Public Const VersionCode As Integer = 441 '正式版
#Else
...
#End If
    ...
    ''' 包含程序名的完整路径。
    ''' </summary>
    Public PathExe As String = If(Environment.ProcessPath, PathExeFolder & AppDomain.CurrentDomain.FriendlyName)
    ''' <summary>
    ''' 程序内嵌图片文件夹路径，以 / 结尾。
    ''' </summary>
    Public PathImage As String = "pack://application:,,,/OmniMixPlayer.Gui.Vbnet;component/Images/"
    ''' <summary>
    ''' 程序的缓存文件夹路径，以 \ 结尾。
    ''' </summary>
...
#Region "文件"
    ...
    Public Function CheckPermission(Folder As String) As Boolean
        Try
            ...
            Return True
        Catch ex As Exception
            Logger.Warn(ex, $"没有对文件夹 {Folder} 的权限，请尝试以管理员权限运行 OmniMix Player")
            Return False
        End Try
    End Function
    ''' <summary>
...
#Region "系统"
    ...
    Public Sub OpenWebsite(Url As String)
        ...
        Catch ex As Exception
            ...
            MyMsgBox("可能由于浏览器未正确配置，OmniMix Player 无法为你打开网页。" & vbCrLf & "网址已经复制到剪贴板，若有需要可以手动粘贴访问。" & vbCrLf &
                     $"网址：{Url}", "无法打开网页")
        End Try
    End Sub
    ''' <summary>
